...
export class ResourceConfigCreator {
  ...
  private createServerConfig(resource: GameResourceModel): RollupOptions {
    const localInstalledPackages = [...Object.keys(this.pkgJson.dependencies || {}), ...Object.keys(this.pkgJson.devDependencies || {})]

    const modulesForConvert = ['@abraham/reflection', 'dotenv', 'fs-extra', 'tsyringe', 'rxjs', 'rxjs/operators']
    ...
    const config = new ResourceConfigModel().cast({
      ...
      plugins: [
        // convertNamedImports(
        //   [...resource.config.convert, ...modulesForConvert, ...this.checkForConvertableModules(localInstalledPackages)],
        //   resource.config,
        // ),
      ],
      external: ['alt-server', ...modulesForConvert, ...localInstalledPackages]
    })
    if (resource.config.external.length > 0) {
      config.external = [...config.external, ...resource.config.external]
    }
  ...
  private checkForConvertableModules(localInstalledPackages: string[]): string[] {
    return localInstalledPackages.filter((localPackage: string) => {
      const localPackageJson = fsJetpack().cwd('node_modules', localPackage).read('package.json', 'json') as PackageJson
