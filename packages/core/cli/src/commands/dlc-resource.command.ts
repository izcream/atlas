...
export const DlcResourceCommand: CommandModule = {
  ...
  async handler(args: Arguments<{ name: string; type: string }>) {
    ...
    switch (args.type) {
      ...
      case 'weapon':
        streamData = generateDirAndFileSetup(weaponTmpFiles(resourceName), weaponStreamCfg(resourceName), baseData(resourceName, args.type))
        break
      ...
      case 'mlo':
        streamData = generateDirAndFileSetup(mloTmpFiles(resourceName), streamCfg, baseData(resourceName, args.type))
        break
      ...
      case 'ped':
        streamData = generateDirAndFileSetup(pedTmpFiles(resourceName), pedStreamCfg(), baseData(resourceName, args.type, false))
        break

      case 'sound':
        streamData = generateDirAndFileSetup(soundTmpFiles(), soundStreamCfg(), baseData(resourceName, args.type, false))
        break
