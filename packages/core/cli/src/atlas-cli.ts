...
async function bootstrap() {
  ...
  if (fsJetpack().exists(pluginFolderName)) {
    ...
    const githubPlugins = fsJetpack().find(pluginFolderName, {
      matching: ['!node_modules', '!node_modules/**', 'package.json'],
      files: true,
      directories: false,
      recursive: true
    })
    ...
    installedPackages.forEach((packageName: string) => {
      ...
      commandExtends.push(fsJetpack().path(packageJsonPath.replace('package.json', ''), installedPackageJson.atlas.commands))
    })
